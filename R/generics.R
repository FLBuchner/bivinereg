...
summary.bivinereg <- function(object, ...) {
  ...
  data.frame(
    ...
    edf = round(object$stats$var_edf, ...),
    cll = round(object$stats$var_cll, ...),
    caic = round(object$stats$var_caic, ...),
    cbic = round(object$stats$var_cbic, ...),
    p_value = formatC(object$stats$var_p_value, format = "e", ...)
  )
}
