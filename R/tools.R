...
#' transforms predicted responses back to original variable scale.
#'
...
to_yscale <- function(u, object) {
  ...
  nms <- colnames(u)
  u[,1] <- vapply(u[,1], qkde1d, numeric(1), obj = object$margins[[1]])
  u[,2] <- vapply(u[,2], qkde1d, numeric(1), obj = object$margins[[2]])
  u <- as.data.frame(u)
  names(u) <- nms
  u
}
